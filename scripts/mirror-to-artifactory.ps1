...
function Write-PinnedValuesYaml {
    ...
    # Each header line is a single-quoted PowerShell string so backticks
    # (PS escape) and dollar signs (PS interpolation) pass through as
    # literal characters into the emitted YAML. The two lines that need
    # variable interpolation ($now / $RepoBase / $Platform) use the
    # -f format operator instead of double-quoted interpolation, which
    # also sidesteps any backtick-in-text gotcha.
    $lines = @(
        '# =============================================================================',
        '# values-pinned.yaml -- digest-pinned image overlay (mirror-emitted)',
        '# =============================================================================',
        ('# Auto-generated by mirror-to-artifactory.ps1 at {0}' -f $now),
        '#',
        ('# Mirror source / destination: {0}' -f $RepoBase),
        ('# Platform pinned: {0}' -f $Platform),
        '#',
        ...
        '# every node pulls the same image content. Eliminates the latest-tag',
        '# content-drift class (different nodes pulling :latest at different',
        '# moments resolved to different shas) that produced the runtime-bundle',
        '# hash-mismatch warnings in diag.sh Section 15.',
        '#',
        '# Usage:',
